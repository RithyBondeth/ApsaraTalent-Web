...
type TGetOneCompanyState = {
    ...
    loading: boolean,
    error: string | null,
    queryCompany: (companyID: string) => Promise<void>,
}
...
export const useGetOneCompanyStore = create<TGetOneCompanyState>((set) => ({
    ...
    loading: false,
    error: null,
    queryCompany: async (companyID: string) => {
        set({ loading: true, error: null });

        try {
            const response = await axios.get<ICompany>(API_GET_ONE_CMP_URL(companyID))
            ...
            if(axios.isAxiosError(error)) 
                ...
                set({ loading: false, error: "An error occurred while fetching one company" })       
        }
    }
}));