...
function ProfileCompletenessCardSkeleton() {
  return (
    <div className="flex w-full items-center gap-4 border border-border border-l-[5px] border-l-foreground bg-card px-5 py-4 shadow-[5px_5px_0_hsl(var(--foreground)/0.055)] sm:gap-6 sm:px-6">
      ...
      <Skeleton className="hidden h-9 w-28 shrink-0 rounded-none xs:block" />
    </div>
  );
}
      ...
      <section className="flex w-full flex-col gap-5">
        ...
        <div className="grid grid-cols-2 gap-3 sm:gap-4 lg:grid-cols-4">
          {Array.from({ length: 4 }).map((_, i) => (
            <div
              key={i}
              className="border border-border border-l-[5px] border-l-foreground bg-card p-4 shadow-[5px_5px_0_hsl(var(--foreground)/0.055)] sm:p-5"
            >
      ...
      <section className="flex w-full flex-col gap-5">
        ...
        <div className="border border-border border-t-[5px] border-t-foreground bg-card p-5 shadow-[5px_5px_0_hsl(var(--foreground)/0.055)] sm:p-6">
          ...
          {Array.from({ length: 5 }).map((_, i) => (
            <div
              key={i}
              className="flex items-center gap-3 border border-border border-l-[3px] border-l-foreground p-3"
            >
